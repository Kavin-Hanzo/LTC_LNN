# quick_test_model.py
# Quick test script for model training, evaluation and experiment slices.
# Usage example:
#   python quick_test_model.py --stage compare --variant all --mode short --experiment e1
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Quick test the modelling and experiment stages without running the full pipeline."
    )
    parser.add_argument(
        "--stage",
...
def get_variant_list(args):
    ...
    if "all" in args.variant:
        variants = _VARIANTS.copy()
    else:
        variants = args.variant
    return sorted(set(variants), key=_VARIANTS.index)


def onehot_map(tickers):
    map_ = {}
    for t in tickers:
        vec = np.zeros(_N_SECTORS, dtype=np.float32)
        vec[_SECTOR_IDX.get(t, 0)] = 1.0
        map_[t] = vec
    return map_
...
def prepare_data(args):
    ...
    vectors, pca, _ = build_vectors(aligned.iloc[:n_tr],
...
def build_dataset_for_variant(processed, vectors, scaled_splits, tickers, lookback, horizon, variant):
    if variant == "baseline":
    ...
    ds = build_datasets(processed, id_map, scaled_splits, tickers, lookback, horizon, ret_idx)
...
def train_variant(model, train_ds, val_ds, args, variant, label, scalers=None):
    model.use_identity = model.use_identity
...
def evaluate_variant(model, dataset, args):
    print("\n[QuickTest Model] Running evaluation")
...
def compare_variants(args, prepared):
    ...
    for variant in variants:
        ...
        # Save prediction plots for each stock and model
        ...
        # Save per-stock metrics to CSV
        metrics_df = pd.DataFrame(per_stock).T
        metrics_csv = os.path.join(args.results_dir, f"metrics_{variant}_{args.label}.csv")
        os.makedirs(args.results_dir, exist_ok=True)
        metrics_df.to_csv(metrics_csv)
        print(f"[Saved] {metrics_csv}")

        # Save aggregated metrics
        agg_csv = os.path.join(args.results_dir, f"agg_metrics_{variant}_{args.label}.csv")
        pd.Series(agg).to_csv(agg_csv, header=False)
        print(f"[Saved] {agg_csv}")

    # E2-style ablation: compare Baseline vs Stock2Vec improvement
    if "baseline" in all_per_stock and "stock2vec" in all_per_stock:
        baseline_metrics = all_per_stock["baseline"]
        stock2vec_metrics = all_per_stock["stock2vec"]
        imp_df = improvement_table(baseline_metrics, stock2vec_metrics)
...
def run_experiments(args, prepared):
    ...
    if "e1" in chosen:
        out = e1_clustering.run(vectors, args.plots_dir, args.results_dir, args.label)
        outputs["e1"] = out

    if "e3" in chosen:
        loo_results = leave_one_out(processed, vectors, scaled_splits, args)
        out = e3_generalization.run(loo_results, vectors, {}, args.plots_dir, args.results_dir, args.label)
        outputs["e3"] = out

    if "e4" in chosen:
        out = e4_interpretability.run(vectors, aligned.iloc[:int(len(aligned) * CFG.training.train_ratio)],
...
def leave_one_out(processed, vectors, scaled_splits, args):
    ...
    for held in tickers:
        ...
        tr_ids = vectors
        ret_idx = FEATURE_COLS.index("close_return")
        tr_ds, va_ds, _ = build_datasets(processed, tr_ids, scaled_splits,
                                        train_tickers, args.lookback, args.horizon, ret_idx)
        _, _, te_ds = build_datasets(processed, tr_ids, scaled_splits,
...
def main():
    ...
    if args.stage in ["train", "evaluate", "compare", "experiment", "all"]:
        print("Preparing data for model quick tests...")
    ...
    if args.stage == "evaluate":
        print(f"Evaluating variant(s): {variants} on split={args.split}")
        raw, processed, aligned, vectors, scaled_splits, scalers = prepared
        for variant in variants:
            if variant == "baseline":
                id_map = {t: zero_vector(CFG.vectors.n_components) for t in processed.keys()}
            ...
            tr_ds, va_ds, te_ds = build_datasets(processed, id_map, scaled_splits,
                                                ...
                                                FEATURE_COLS.index("close_return"))
